 ...
 *
 */
public class ConfigVectorFileDsNewWindow extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ConfigVectorFileDsNewWindow frame = new ConfigVectorFileDsNewWindow(new MainWindow());
	 ...
	 * Create the frame.
	 */
	public ConfigVectorFileDsNewWindow(MainWindow mainWindow) {
		this.sourceType = "矢量文件（新格式）";
	...
	public void RefreshFieldListTable(int row) {
		...
		if (vfList != null && vfList.size() > 0) {
			...
			for (int i = 0; i < fieldNum; i++) {
				...
				if (className.endsWith("Point") || className.endsWith("MultiPoint") || className.endsWith("LineString")
						|| className.endsWith("MultiLineString") || className.endsWith("Polygon")
						|| className.endsWith("MultiPolygon")) {
...
//				File file = JFileDataStoreChooser.showOpenFile("shp", null);
	...
	public JButton selectDire() {
		...
		btnNewButton_1.setBounds(536, 24, 93, 23);
	...
	public JLabel newJLabel2() {
		...
		lblNewLabel_1.setBounds(400, 66, 74, 15);
	...
	public JScrollPane inputList() {
		...
		table_1.getColumnModel().getColumn(0).setPreferredWidth(141);
	...
	public JScrollPane outputList() {
		...
		jScrollPane_2.setBounds(400, 91, 323, 466);
	...
	public JButton confirm() {
		...
		btnNewButton_2.setBounds(846, 587, 93, 23);
...
//		List<InputFieldListArgs> fieldToTypeList = getFieldToTypeList(path); // 获取字段列表
		...
		if (charset != null) {
			comboBox.setSelectedItem(charset);
		} else {
			comboBox.setSelectedItem("GBK");
		}
...
//			table_1.setRowSelectionInterval(focusedRowIndex, focusedRowIndex);
	...
	public void addCharset() {
		...
		label.setBounds(773, 66, 131, 15);
		contentPane.add(label);

		comboBox = new JComboBox();
		comboBox.setBounds(773, 89, 166, 21);
		...
		// 初始化数据
		if (vfList.size() > 0) {
			comboBox.setSelectedItem(vfList.get(vfList.size() - 1).getCharset());
		}
	}
	...
	public static void setVfList(List<InputListArgs> vfList) {
		ConfigVectorFileDsNewWindow.vfList = vfList;
	}
}
